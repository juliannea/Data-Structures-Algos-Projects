...
//should we replace the dafulat values to be 1-4 since action selection takes 1-4??
enum Action{
 ...
 */
 std::string getActionString(int num);
 ...
 */
 void applyActions(Character* target);
...
private:
  ...
  std::string name_;
  // he race of the character (an enum)
  Race race_;
  ...
  bool enemy_;
  //Action queue: a queue of integers to store actions a character wants to take during their turn (as integers that correspond to the enum Action) 
